...
var tool = require('../middleware/tool.server.middleware');
var multer = require('multer');
var shortid = require('shortid');
const ROOT_PATH = require('../config/constant.server.config').PROJECT_ROOT_PATH;
...
var storageImg = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        if (req.query.uniqueId) {
            var dirPathParent = path.join(ROOT_PATH, '/www/uploads/recipe/', req.query.uniqueId),
...
router.get('/', function (req, res, next) {
    ...
    switch (req.query.action) {
        //获取配置
        case 'config':
            tool.getConfig(path.join(ROOT_PATH, '/src/config/prop/ue.json'), function (err, settings) {
                if (err) {
                    next(err);
                } else {
                    res.json(settings);
